...
export class ObjectStorageService {
  ...
  async getObjectEntityUploadURL(): Promise<string> {
    ...
    // Check if we are on Replit. If not, use local storage fallback.
    if (!process.env.REPL_ID) {
      // Return a local URL that our server will handle
      // The client will then PUT to this URL
      const host = process.env.PUBLIC_URL || "http://localhost:3000";
      return `${host}/api/storage/local-upload/${objectId}`;
    }
  ...
  normalizeObjectEntityPath(rawPath: string): string {
    ...
    try {
      ...
      // If no private dir, we can't strip it
